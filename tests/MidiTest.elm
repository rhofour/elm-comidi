...
fuzzMidiFileEvent =
    Fuzz.oneOf commonEvents


generateMidiMessage : Generator MidiMessage
generateMidiMessage =
    Random.pair (Random.int 0 1023) generateMidiFileEvent



--Random.pair (Random.constant 100) generateMidiFileEvent


fuzzMidiMessage : Fuzzer MidiMessage
fuzzMidiMessage =
    Fuzz.tuple ( (intRange 0 2048), fuzzMidiFileEvent )
...
fuzzTrack =
    Fuzz.andThen
        (\numEvents -> listOfLength fuzzMidiMessage numEvents)
        (intRange 0 1)
...
simpleRecording : MidiRecording
simpleRecording =
    let
        header =
            { formatType = 0, ticksPerBeat = 32 }
...
suite =
    describe "MIDI tests"
        ...
        , test "Go from simple MidiRecording to \"Binary\" and back" <|
            \_ ->
                Expect.equal
                    (Ok simpleRecording)
                    (parse (toByteString (Generate.recording simpleRecording)))
        , only <|
